    ...
    <div className="w-[1280px] h-[720px] bg-white p-8 font-sans flex flex-col">
      ...
      >
        <div>
          ...
          <p className="text-base text-gray-500">Análisis de la Caída · <span className="text-[#F7B500] font-medium">Sep 2025 - Ene 2026 vs Año Anterior</span></p>
        </div>

        {/* Badge de caída */}
        <div
          className={`px-4 py-2 rounded-xl bg-red-50 border-2 border-red-200 transition-all duration-500 ${
            isLoaded ? "opacity-100 scale-100" : "opacity-0 scale-90"
          }`}
          style={{ transitionDelay: "200ms" }}
        >
          <div className="flex items-center gap-2">
            <TrendingDown size={20} className="text-[#C0392B]" />
            <span className="text-2xl font-bold text-[#C0392B]">{diagnostico.variacion}%</span>
          </div>
          <p className="text-xs text-red-700">vs año anterior</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex gap-4">
        ...
        <div className="flex-1 flex flex-col">
          ...
          >
            <div className="flex-1 p-3 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <p className="text-xl font-bold text-gray-700">{diagnostico.anterior.toLocaleString()}</p>
            ...
            <div className="flex-1 p-3 bg-red-50 rounded-lg border-2 border-red-200">
              <p className="text-xs text-red-600">Actual</p>
              <p className="text-xl font-bold text-[#C0392B]">{diagnostico.actual.toLocaleString()}</p>
            ...
            <div className="flex-1 p-3 bg-red-100 rounded-lg border-2 border-red-300">
              <p className="text-xs text-red-700">Pérdida</p>
              <p className="text-xl font-bold text-[#C0392B]">{diagnostico.diferencia.toLocaleString()}</p>
          ...
          >
            ...
            <div className="space-y-1">
              {plazasCaida.map((item, index) => (
                <div
                  key={item.plaza}
                  className={`flex items-center gap-2 py-1 px-2 rounded-lg cursor-pointer transition-all duration-300 ${
                ...
                >
                  {/* Plaza name */}
                  <span className="w-32 text-xs font-medium text-gray-700 truncate">
                    {item.plaza}
                  </span>
                  ...
                  {/* Percentage */}
                  <span className="w-14 text-right text-xs font-bold text-[#C0392B]">
                    {item.var}%
                  </span>
                </div>
              ))}
            ...
            <div
              className={`mt-3 p-3 bg-red-50 border-2 border-red-200 rounded-lg transition-all duration-700 ${
                isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
              }`}
        ...
        <div className="w-[280px] flex flex-col gap-3">
          ...
          <div
            className={`p-3 bg-green-50 border-2 border-green-300 rounded-xl transition-all duration-500 hover:border-green-500 hover:shadow-lg cursor-pointer ${
              isLoaded ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4"
            }`}
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-green-800 text-sm">Lo Positivo</span>
            </div>
            <p className="text-base font-bold text-[#1A1A1A]">{positivo.plaza}</p>
            <div className="mt-1 flex items-baseline gap-2">
              <span className="text-xl font-bold text-[#27AE60]">+{positivo.var}%</span>
              <span className="text-xs text-green-700">crecimiento</span>
            </div>
            <div className="mt-1 flex items-center gap-2 text-xs text-green-700">
              <TrendingUp size={12} />
              <span>{positivo.anterior} → {positivo.actual} unidades</span>
            </div>
            <p className="text-xs text-green-600 mt-1 font-medium">{positivo.nota}</p>
          </div>

          {/* Insight card */}
          <div
            className={`p-3 bg-amber-50 border-2 border-amber-200 rounded-xl transition-all duration-500 hover:border-amber-400 hover:shadow-lg cursor-pointer ${
              isLoaded ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4"
            }`}
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-amber-800 text-sm">Insight</span>
            </div>
          ...
          {/* Recommendation card */}
          <div
            className={`p-3 bg-blue-50 border-2 border-blue-200 rounded-xl transition-all duration-500 hover:border-blue-400 hover:shadow-lg cursor-pointer flex-1 ${
              isLoaded ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4"
            }`}
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-blue-800 text-sm">Recomendación</span>
            </div>
